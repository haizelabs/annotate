...
class InteractionGroup(BaseModel):
    """A group of related interactions (e.g., a session, user, or experiment)."""
...
class FeedbackConfig(BaseModel):
    """
    Unified configuration that defines WHAT to evaluate and HOW to present it.
    Used by both human annotators and AI judges to ensure consistent evaluation criteria.
    """

    id: str = Field(
        description="ID of the feedback config - should be auto-generated each time the config changes"
    )
    granularity: Literal["step", "interaction", "group"] = Field(
        ...
        description="""The granularity of raw data passed in for evaluation:
            ...
            'group' for session-level evaluation,
                example: evaluating whether the agent helped accomplish the user's goal in a chat session""",
    )
    ...
    attribute_matchers: list[AttributeMatcher] = Field(
        ...
        include data that is obviously noisy and irrelevant. The attribute matchers' `match` function takes ANY OF an interaction step, interaction, or group of interactions
        as input; keep that in mind when designing matchers. It must be in sync with `granularity` described in the feedback config.
        """
    )
...
class BaseAnnotation(BaseModel):
    ...
    annotator_id: str = Field(
        description="Username or model name (e.g., 'human' or 'claude-sonnet-4')"
    )
